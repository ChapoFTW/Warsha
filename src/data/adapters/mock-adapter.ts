...
import { syncMockProviderCatalog } from '@/src/providers/mock-provider-catalog';

export const mockDataAdapter: WarshaDataAdapter = {
  mode: 'mock',
  async listCategories() { return categories; },
  async listProviders() { await syncMockProviderCatalog(); return providers.filter((provider) => provider.verified); },
  async getProvider(id) { await syncMockProviderCatalog(); const provider = getProvider(id); return provider?.verified ? provider : undefined; },

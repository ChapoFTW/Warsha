...
export type AddressPin={latitude:number;longitude:number;pinSource:NonNullable<Address['pinSource']>};
export interface AddressRepository{list():Promise<Address[]>;add(input:Omit<Address,'id'>):Promise<Address>;update(id:string,input:Omit<Address,'id'>):Promise<Address>;remove(id:string):Promise<void>;setDefault(id:string):Promise<void>;confirmPin(id:string,pin:AddressPin):Promise<Address>}
...
export type CustomerProfile={id:string;displayName:string;preferredLanguage:SupportedLanguage;phone:string};

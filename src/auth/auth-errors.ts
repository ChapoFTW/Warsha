...
function classify(error: unknown): AuthFailure {
  ...
  if (status >= 500 || code === 'unexpected_failure') return 'authServerError';
...
export function safeAuthDiagnostic(operation: AuthOperation, error: unknown) {
  ...
  return {
    ...
    retryable: failure === 'authNetworkError' || failure === 'authServerError' || failure === 'authRateLimited',
  };
}

export function sanitizeAuthError(error: unknown, operation: AuthOperation = 'unknown'): SafeAuthError {
  const safe = error instanceof SafeAuthError ? error : new SafeAuthError(classify(error));

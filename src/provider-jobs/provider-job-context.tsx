...
export function ProviderJobsProvider({children}:PropsWithChildren){
  const provider=useProviderFoundation();const auth=useAuth();const[jobs,setJobs]=useState<Booking[]>([]);const[loading,setLoading]=useState(true);const[refreshing,setRefreshing]=useState(false);const[error,setError]=useState<TranslationKey|null>(null);const[actionInFlight,setActionInFlight]=useState<string|null>(null);const locks=useRef(new Set<string>());
  ...
  useEffect(()=>{const providerId=provider.profile?.id;if(!providerId||(auth.mode==='supabase'&&!auth.user))return;let first=true;let timer:ReturnType<typeof setTimeout>|undefined;const reconcile=()=>{if(timer)return;timer=setTimeout(()=>{timer=undefined;void reload(true)},120)};const unsubscribe=realtimeService.providerJobs(providerId,reconcile,status=>{if(status==='connected'){if(first)first=false;else reconcile()}});return()=>{if(timer)clearTimeout(timer);unsubscribe()}},[auth.mode,auth.user,provider.profile?.id,reload]);

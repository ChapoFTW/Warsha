    ...
    <View style={styles.card}>
      ...
      {loading ? <ActivityIndicator accessibilityLabel={rt('loading')} color={colors.white} /> : review && !editing ? (
        ...
        <View style={styles.form}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel={review ? rt('saveChanges') : rt('submit')} accessibilityState={{ disabled: reviews.busy, busy: reviews.busy }} disabled={reviews.busy} onPress={() => void save()} style={[styles.button, reviews.busy && styles.disabled]}>{reviews.busy ? <ActivityIndicator color={colors.background} /> : <AppText style={styles.buttonText}>{review ? rt('saveChanges') : rt('submit')}</AppText>}</Pressable>
        </View>
      ) : null}

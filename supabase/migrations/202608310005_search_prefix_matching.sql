-- "electrical" found the electrician. "electric" found nobody.
--
-- `websearch_to_tsquery` produces whole lexemes, so the search only ever
-- matched complete words -- and a complete word is not what a person has typed
-- while they are still typing it. Every partial query returned an empty list,
-- in all three languages.
--
-- This adds a prefix query alongside the exact one and matches on either. The
-- exact query is unchanged and still runs first, so a correctly spelled search
-- behaves exactly as it did; the prefix query only ever adds rows.
--
-- The user's text is stripped of every character `tsquery` treats as an
-- operator before it is parsed, and a query that still will not parse is
-- discarded rather than raised -- the exact match stands on its own. Terms
-- shorter than two characters are dropped, because "a:*" matches a large part
-- of the catalogue and means nothing.
--
-- Nothing is whitelisted by alphabet. A whitelist that has to name every script
-- is a whitelist that will forget Arabic.

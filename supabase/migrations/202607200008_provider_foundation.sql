...
create or replace function public.save_provider_foundation(p_profile pg_catalog.jsonb,p_submit pg_catalog.boolean default false)
...
begin
  ...
  if pg_catalog.length(pg_catalog.btrim(pg_catalog.coalesce(p_profile->>'displayName',''))) not between 2 and 100 then raise exception 'Invalid provider information' using errcode='22023'; end if;
  if pg_catalog.length(pg_catalog.btrim(pg_catalog.coalesce(p_profile->>'profession',''))) not between 2 and 100 then raise exception 'Invalid provider information' using errcode='22023'; end if;
  if pg_catalog.coalesce((p_profile->>'experienceYears')::pg_catalog.integer,-1) not between 0 and 80 then raise exception 'Invalid provider information' using errcode='22023'; end if;
  radius:=pg_catalog.coalesce((p_profile->>'serviceRadiusKm')::pg_catalog.numeric,0); if radius not between 1 and 250 then raise exception 'Invalid provider information' using errcode='22023'; end if;
  if p_submit and pg_catalog.coalesce((p_profile->>'agreementAccepted')::pg_catalog.boolean,false)=false then raise exception 'Provider agreement is required' using errcode='22023'; end if;
  if exists(select 1 from pg_catalog.jsonb_array_elements_text(pg_catalog.coalesce(p_profile->'categoryIds','[]'::pg_catalog.jsonb)) c where not exists(select 1 from public.service_categories sc where sc.id=c.value and sc.is_active and sc.deleted_at is null)) then raise exception 'Invalid service category' using errcode='22023'; end if;
  if (select pg_catalog.count(*) from pg_catalog.jsonb_array_elements(pg_catalog.coalesce(p_profile->'services','[]'::pg_catalog.jsonb)))<>(select pg_catalog.count(distinct value->>'serviceId') from pg_catalog.jsonb_array_elements(pg_catalog.coalesce(p_profile->'services','[]'::pg_catalog.jsonb))) then raise exception 'Duplicate provider service' using errcode='22023'; end if;
  if exists(select 1 from pg_catalog.jsonb_array_elements(pg_catalog.coalesce(p_profile->'areas','[]'::pg_catalog.jsonb)) a group by a.value->>'governorate',a.value->>'district' having pg_catalog.count(*)>1) then raise exception 'Duplicate service area' using errcode='22023'; end if;
  ...
  update public.provider_profiles set display_name=pg_catalog.btrim(p_profile->>'displayName'),avatar_url=pg_catalog.nullif(p_profile->>'avatarUrl',''),profession_key=pg_catalog.btrim(p_profile->>'profession'),about=pg_catalog.left(pg_catalog.coalesce(p_profile->>'about',''),2000),experience_years=(p_profile->>'experienceYears')::pg_catalog.integer,languages=pg_catalog.coalesce(array(select pg_catalog.jsonb_array_elements_text(p_profile->'languages')),'{}'),skills=pg_catalog.coalesce(array(select pg_catalog.jsonb_array_elements_text(p_profile->'skills')),'{}'),category_ids=pg_catalog.coalesce(array(select pg_catalog.jsonb_array_elements_text(p_profile->'categoryIds')),'{}'),primary_category_id=pg_catalog.nullif(p_profile->'categoryIds'->>0,''),service_radius_km=radius,is_available=pg_catalog.coalesce((p_profile->>'isOnline')::pg_catalog.boolean,false),emergency_available=pg_catalog.coalesce((p_profile->>'emergencyAvailable')::pg_catalog.boolean,false),temporary_unavailable_until=pg_catalog.nullif(p_profile->>'temporaryUnavailableUntil','')::pg_catalog.timestamptz,provider_agreement_accepted_at=case when pg_catalog.coalesce((p_profile->>'agreementAccepted')::pg_catalog.boolean,false) then pg_catalog.coalesce(provider_agreement_accepted_at,pg_catalog.now()) else provider_agreement_accepted_at end,onboarding_status=next_status where id=pid and user_id=uid;
  ...
  for item in select * from pg_catalog.jsonb_array_elements(pg_catalog.coalesce(p_profile->'services','[]'::pg_catalog.jsonb)) loop
    service_price:=pg_catalog.coalesce((item->>'price')::pg_catalog.numeric,-1); transportation_fee:=pg_catalog.coalesce((item->>'transportationFee')::pg_catalog.numeric,-1); emergency_fee:=pg_catalog.coalesce((item->>'emergencySurcharge')::pg_catalog.numeric,-1);
  ...
  for item in select * from pg_catalog.jsonb_array_elements(pg_catalog.coalesce(p_profile->'areas','[]'::pg_catalog.jsonb)) loop
    area_radius:=pg_catalog.coalesce((item->>'radiusKm')::pg_catalog.numeric,0); if area_radius not between 1 and 250 or pg_catalog.length(pg_catalog.btrim(pg_catalog.coalesce(item->>'governorate','')))=0 then raise exception 'Invalid service area' using errcode='22023'; end if;
    insert into public.provider_service_areas(provider_id,governorate,district,radius_km) values(pid,pg_catalog.left(pg_catalog.btrim(item->>'governorate'),100),pg_catalog.left(pg_catalog.btrim(pg_catalog.coalesce(item->>'district','')),100),area_radius);
  end loop;
  delete from public.provider_availability where provider_id=pid and weekday is not null;
  for item in select * from pg_catalog.jsonb_array_elements(pg_catalog.coalesce(p_profile->'availability','[]'::pg_catalog.jsonb)) where pg_catalog.coalesce((value->>'enabled')::pg_catalog.boolean,false) loop
    insert into public.provider_availability(provider_id,weekday,start_time,end_time,break_start,break_end) values(pid,(item->>'weekday')::pg_catalog.int2,(item->>'startTime')::pg_catalog.time,(item->>'endTime')::pg_catalog.time,pg_catalog.nullif(item->>'breakStart','')::pg_catalog.time,pg_catalog.nullif(item->>'breakEnd','')::pg_catalog.time);
  end loop;
exception
  when sqlstate '42501' or sqlstate '22023' then raise;

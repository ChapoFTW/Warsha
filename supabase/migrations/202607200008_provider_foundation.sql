...
declare uid uuid:=(select auth.uid()); result_id uuid;
begin
  if uid is null then raise exception 'Authentication required' using errcode='42501'; end if;
  if length(trim(p_display_name)) not between 2 and 100 then raise exception 'Invalid display name' using errcode='22023'; end if;
  ...
  values(uid,trim(p_display_name),'professional','draft',false,false)
  on conflict(user_id) where user_id is not null do update set user_id=excluded.user_id
  returning id into result_id;
  return result_id;
end;
$$;
revoke all on function public.activate_provider_role(text) from public;
grant execute on function public.activate_provider_role(text) to authenticated;

create or replace function public.save_provider_foundation(p_profile jsonb,p_submit boolean default false)
returns void language plpgsql security definer set search_path='' as $$
declare uid uuid:=(select auth.uid()); pid uuid; item jsonb; service_price numeric; radius numeric; next_status text;
begin
  ...
  if length(trim(coalesce(p_profile->>'displayName',''))) not between 2 and 100 then raise exception 'Invalid display name' using errcode='22023'; end if;
  if length(trim(coalesce(p_profile->>'profession',''))) not between 2 and 100 then raise exception 'Invalid profession' using errcode='22023'; end if;
  if coalesce((p_profile->>'experienceYears')::integer,-1) not between 0 and 80 then raise exception 'Invalid experience' using errcode='22023'; end if;
  radius:=coalesce((p_profile->>'serviceRadiusKm')::numeric,0); if radius not between 1 and 250 then raise exception 'Invalid service radius' using errcode='22023'; end if;
  ...
  if exists(select 1 from jsonb_array_elements_text(coalesce(p_profile->'categoryIds','[]')) c where not exists(select 1 from public.service_categories sc where sc.id=c.value and sc.is_active and sc.deleted_at is null)) then raise exception 'Invalid service category' using errcode='22023'; end if;
  ...
  update public.provider_profiles set display_name=trim(p_profile->>'displayName'),avatar_url=nullif(p_profile->>'avatarUrl',''),profession_key=trim(p_profile->>'profession'),about=left(coalesce(p_profile->>'about',''),2000),experience_years=(p_profile->>'experienceYears')::integer,languages=coalesce(array(select jsonb_array_elements_text(p_profile->'languages')),'{}'),skills=coalesce(array(select jsonb_array_elements_text(p_profile->'skills')),'{}'),category_ids=coalesce(array(select jsonb_array_elements_text(p_profile->'categoryIds')),'{}'),primary_category_id=nullif(p_profile->'categoryIds'->>0,''),service_radius_km=radius,is_available=coalesce((p_profile->>'isOnline')::boolean,false),emergency_available=coalesce((p_profile->>'emergencyAvailable')::boolean,false),temporary_unavailable_until=nullif(p_profile->>'temporaryUnavailableUntil','')::timestamptz,provider_agreement_accepted_at=case when coalesce((p_profile->>'agreementAccepted')::boolean,false) then coalesce(provider_agreement_accepted_at,now()) else provider_agreement_accepted_at end,onboarding_status=next_status where id=pid;
  delete from public.provider_services where provider_id=pid;
  for item in select * from jsonb_array_elements(coalesce(p_profile->'services','[]')) loop
    service_price:=coalesce((item->>'price')::numeric,-1); if service_price not between 0 and 100000 then raise exception 'Invalid service price' using errcode='22023'; end if;
    insert into public.provider_services(provider_id,service_id,custom_price_egp,pricing_type,transportation_fee_egp,emergency_surcharge_egp,is_active) select pid,(item->>'serviceId')::uuid,service_price,item->>'pricingModel',coalesce((item->>'transportationFee')::numeric,0),coalesce((item->>'emergencySurcharge')::numeric,0),true from public.services s where s.id=(item->>'serviceId')::uuid and s.is_active and s.deleted_at is null;
  ...
  for item in select * from jsonb_array_elements(coalesce(p_profile->'areas','[]')) loop
    insert into public.provider_service_areas(provider_id,governorate,district,radius_km) values(pid,left(item->>'governorate',100),left(coalesce(item->>'district',''),100),least(greatest((item->>'radiusKm')::numeric,1),250));
  end loop;
  delete from public.provider_availability where provider_id=pid and weekday is not null;
  for item in select * from jsonb_array_elements(coalesce(p_profile->'availability','[]')) where coalesce((value->>'enabled')::boolean,false) loop
    insert into public.provider_availability(provider_id,weekday,start_time,end_time,break_start,break_end) values(pid,(item->>'weekday')::smallint,(item->>'startTime')::time,(item->>'endTime')::time,nullif(item->>'breakStart','')::time,nullif(item->>'breakEnd','')::time);
  end loop;
end;
$$;
revoke all on function public.save_provider_foundation(jsonb,boolean) from public;

...
from provider_seed on conflict(id) do update set display_name=excluded.display_name,is_published=true,location_label=excluded.location_label,rating_average=excluded.rating_average;
...
select p.id,s.id,s.price_egp+(p.n*5) from provider_seed p join lateral(select id,price_egp from public.services order by id limit 1 offset ((p.n-1)%5)) s on true

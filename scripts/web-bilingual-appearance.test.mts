...
import { directionOf, localeFromAcceptLanguage, otherLocale } from '../web/lib/preferences.ts';
...
equal(localeFromAcceptLanguage('fr-FR'), 'en', 'an unsupported language falls back to English');
...
const allowedIdentical = new Set(['languageEnglish', 'languageArabic']);
for (const key of enKeys) {
...
equal(otherLocale('ar'), 'en', 'the switcher offers the other language');
equal(otherLocale('en'), 'ar', 'the switcher offers the other language both ways');
...
check(/hrefLang=\{target\}/.test(controls),

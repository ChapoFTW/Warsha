...
| TRS-10 | Nobody can act on a review report | OPEN (P1) | Administration is web-only; staff authority `moderate_reviews` | Customers and Professionals can report a review (native). The server has the queue (`get_staff_queue('review_moderation')`), hide/restore and report transitions, audited — and no web admin page calls any of them, so reports wait forever and a staff member cannot hide an abusive review | not deployed | not verified | Web admin moderation page on the existing RPCs and staff RLS reads, EN/AR/FR | No |

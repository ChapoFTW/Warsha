...
| TRS-07 | A restricted person can see their status and appeal | OPEN (P1) | Owner: removed/banned keep status, reason, support and appeal | `submit_trust_appeal` existed but no read returned the action id it needs; `get_my_trust_status` now does (`202609170006`, tested). **No native or web screen reads trust status or offers an appeal** | — | — | Build the status and appeal surface on both platforms, with render proof | No |

...
| TRS-09 | A Professional's rating, review count and completed jobs are never kept | OPEN (P1) | Canonical: `private.provider_reputation` (visible, undeleted reviews; completed bookings) | `provider_profiles.rating_average`, `review_count` and `completed_jobs` are guarded against edits (`prevent_provider_approval_changes`) and written by nothing in any migration. Matching score, the new-Professional fairness boost, quote cards (`workerRating`, `workerReviewCount`, completed jobs), search sort and the minimum-rating filter all read them, so every real Professional reads as 0.0 with no jobs however many reviews they have. Fixtures set the columns directly, which hid it | not deployed | not verified by query | Keep the three columns from the same rule as the summary; backfill; pgTAP with mutation | No |
